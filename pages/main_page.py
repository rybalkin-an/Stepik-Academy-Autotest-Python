...
class MainPage(BasePage):
    ...
    def should_be_menu_in_ru_language(self):
        assert self.is_element_present(*MainPageLocators.CATALOG_MENU_ALL_ITEMS), \
            "All items in RU catalog menu is not presented"
        assert self.is_element_present(*MainPageLocators.CATALOG_MENU_CLOTHING), \
            "Clothing in RU catalog menu is not presented"
        assert self.is_element_present(*MainPageLocators.CATALOG_MENU_BOOKS), \
            "Books in RU catalog menu is not presented"
        assert self.is_element_present(*MainPageLocators.CATALOG_MENU_OFFERS), \
